...
class CameraProcessor:
    def __init__(self, camera_id, width=640, height=480):
        ...
        # We'll create a separate hands instance for each camera
        # but make sure we properly configure it to focus on detection
    ...
    def start(self):
        ...
        self.hands = mp_hands.Hands(
            ...
            min_detection_confidence=0.5,  # Set higher for more reliable detection
            min_tracking_confidence=0.5,  # Set higher for more reliable tracking
            model_complexity=1  # Use moderate complexity for better detection but still reasonable performance
        )
...
def optimize_gif_size(frames, target_size_mb=10, initial_quality=100, min_quality=30):
    ...
    while quality >= min_quality:
        ...
        for frame in frames[:5]:  # Use first 5 frames to estimate
            if resize_factor < 1.0:
                ...
                test_frame = frame.resize(new_size, Image.LANCZOS if hasattr(Image, 'LANCZOS') else Image.ANTIALIAS)
            else:
    ...
    for frame in frames:
        if resize_factor < 1.0:
            new_size = (int(frame.width * resize_factor), int(frame.height * resize_factor))
            optimized_frame = frame.resize(new_size, Image.LANCZOS if hasattr(Image, 'LANCZOS') else Image.ANTIALIAS)
        else:
...
def main():
    # Set MediaPipe resource caching directory to help with multiple instances
    cache_dir = os.path.join(os.path.expanduser('~'), '.mediapipe_cache')
    os.makedirs(cache_dir, exist_ok=True)
    os.environ['MEDIAPIPE_RESOURCE_CACHE_DIR'] = cache_dir
    ...
    # Create and start camera processors with delay between each to avoid resource contention
    camera_processors = []
    for camera_id in available_cameras:
        processor = CameraProcessor(camera_id)
        if processor.is_connected:
            print(f"Initializing camera {camera_id} for hand tracking...")
            started = processor.start()
            if started:
                camera_processors.append(processor)
                print(f"Started hand tracking for camera {camera_id}")
                # Add a delay between starting each camera to reduce resource contention
                time.sleep(2.0)  # Increased delay to give each camera time to initialize properly
            else:
    ...
    frame_capture_interval = 3  # Capture every 3rd frame to reduce GIF size
    frame_counter = 0
    ...
    # Main display loop
    try:
        print(f"Successfully started {len(camera_processors)} camera processors with hand tracking")
        ...
        while True:
            ...
            for i, processor in enumerate(camera_processors):
                ...
                hand_status = "Hands Detected" if processor.is_hand_detected() else "No Hands"
